...
public class BeneficiaryController {
	 ...
	 *            the user input Beneficiary details
	...
	private ResponseEntity<StatusInfo> addBeneficiary(@RequestBody @Valid BeneficiaryDto beneficiaryDto,
			Authentication authentication, Locale locale, BindingResult bindingResult) {
		try {
			logger.info(Utility.ENTERED + new Object() {
			}.getClass().getEnclosingMethod().getName());
			StatusInfo responceObj = beneficiaryService.addBeneficiary(beneficiaryDto, authentication, locale,
					bindingResult);
	...
	// Anju added 16/11/2018
	@RequestMapping(value = "/domesticaddbeneficiary", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE)
	private ResponseEntity<StatusInfo> addDomesticBeneficiary(
			@RequestBody @Valid DomesticBeneficiaryReqDto domesticBeneficiaryDto, Authentication authentication,
			Locale locale, BindingResult bindingResult) {
		try {
			logger.info(Utility.ENTERED + new Object() {
			}.getClass().getEnclosingMethod().getName());
			StatusInfo responceObj = beneficiaryService.addDomesticBeneficiary(domesticBeneficiaryDto, authentication,
	...
	private ResponseEntity<PayeeInfo> viewBeneficiary(Authentication authentication, Locale locale) {
		try {
			...
			PayeeInfo responceObj = beneficiaryService.viewBeneficiary(authentication, locale, "DOM");
			if (responceObj.getErrorStatus()) {
				logger.info(Utility.EXITING + new Object() {
				}.getClass().getEnclosingMethod().getName());
	...
	// Anju added 20/11/2018
	@RequestMapping(value = "/fetch-config-nw-mode", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	private ResponseEntity<ConfigDtls> fetchPaymentMode(Authentication authentication, Locale locale) {
		try {
			logger.info(Utility.ENTERED + new Object() {
			}.getClass().getEnclosingMethod().getName());
			ConfigDtls responceObj = beneficiaryService.fetchPaymentMode(authentication, locale);
			if (responceObj.getErrorStatus()) {
				logger.info(Utility.EXITING + new Object() {
				}.getClass().getEnclosingMethod().getName());
				return new ResponseEntity<ConfigDtls>(responceObj, HttpStatus.BAD_REQUEST);
			}
			logger.info(Utility.EXITING + new Object() {
			}.getClass().getEnclosingMethod().getName());
			return new ResponseEntity<ConfigDtls>(responceObj, HttpStatus.OK);
		} catch (Exception exception) {
			logger.info(Utility.EXCEPTION_IN + new Object() {
			}.getClass().getEnclosingMethod().getName());
			return new ResponseEntity<ConfigDtls>(HttpStatus.INTERNAL_SERVER_ERROR);
		}
